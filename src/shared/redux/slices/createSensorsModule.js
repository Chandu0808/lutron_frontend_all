...
export function createSensorsModule({ BaseUrl }) {

  // Async thunk for fetching sensors
  const fetchSensors = createAsyncThunk(
  ...
  const discoverSensors = createAsyncThunk(
  ...
  const {  clearError, clearDiscoverError, clearDiscoverSuccess, resetSensorsState  } = sensorsSlice.actions;
  const reducer = sensorsSlice.reducer;
  return {
    reducer,
    fetchSensors,
    discoverSensors,
