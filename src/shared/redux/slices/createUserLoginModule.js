...
export function createUserLoginModule({ BaseUrl, clearDashboardData, clearHeatmapUserData, clearAlertsState }) {
  ...
  // 5) Selectors
  const selectSigninData = (state) => state.user.signinData;
  const selectLoading = (state) => state.user.loading;
  const selectError = (state) => state.user.error;
  const selectProfile = (state) => state.user.profile;
  const selectProfileLoading = (state) => state.user.profileLoading;
  const selectProfileError = (state) => state.user.profileError;
  const selectLogoutLoading = (state) => state.user.logoutLoading;
  const selectLogoutError = (state) => state.user.logoutError;
  const selectChangePasswordLoading = (state) => state.user.changePasswordLoading;
  const selectChangePasswordError = (state) => state.user.changePasswordError;
  const selectChangePasswordSuccess = (state) => state.user.changePasswordSuccess;

  // 6) Actions
  const {  resetChangePasswordState  } = userSlice.actions;
