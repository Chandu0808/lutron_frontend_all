...
export function createModulesModule({ BaseUrl }) {

  // Async thunk for fetching modules
  const fetchModules = createAsyncThunk(
  ...
  const uploadDeviceAlerts = createAsyncThunk(
  ...
  const {  clearError, clearUploadError, clearUploadSuccess, resetModulesState  } = modulesSlice.actions;
  const reducer = modulesSlice.reducer;
  return {
    reducer,
    fetchModules,
    uploadDeviceAlerts,
