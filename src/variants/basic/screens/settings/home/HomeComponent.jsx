 ...
 * - Admin: Home, Theme, Area Groups, Area Size for Energy, Email Server, User Management
 *   (see getVisibleSidebarItems in utils/sidebarItems.jsx)
 * - Operator: Area Groups, Area Size for Energy, User Management (and variants per profile;
 *   see getVisibleSidebarItems)
 * 
...
const HomeComponent = () => {
    ...
    const displayMode = useMemo(() => {
        const raw = (searchParams.get(SETTINGS_HOME_TAB_QUERY) || 'lutron').toLowerCase();
        const map = { lutron: 'Lutron', client: 'Client', project: 'Project' };
        return map[raw] || 'Lutron';
    }, [searchParams]);
    ...
    const handleHomeTabChange = (mode) => {
        setSearchParams(
            (prev) => {
                const next = new URLSearchParams(prev);
                next.set(SETTINGS_HOME_TAB_QUERY, String(mode).toLowerCase());
                return next;
            },
            { replace: true }
        );
            ...
            {HOME_TAB_KEYS.map((mode) => (
                <Button
                    ...
                    sx={{
                        ...
                        backgroundColor: displayMode === mode ? '#fff' : settingsAccentColor,
                        color: displayMode === mode ? settingsAccentColor : '#fff',
                        borderRadius: '50%',
                        fontWeight: 'bold',
                        border: `1px solid ${settingsAccentColor}`,
                        textTransform: 'none',
                        minWidth: 'unset',
                        padding: 0,
                        fontSize: { xs: '8px', sm: '10px', md: '12px', lg: '14px' },
                        '&:hover': {
                            backgroundColor: displayMode === mode ? '#f5f5f5' : darken(settingsAccentColor, 0.12),
                        },
                        '&:focus': {
                            backgroundColor: displayMode === mode ? '#fff' : settingsAccentColor,
                            color: displayMode === mode ? settingsAccentColor : '#fff',
                        ...
                        '&:active': {
                            backgroundColor: displayMode === mode ? '#ededed' : darken(settingsAccentColor, 0.18),
                            color: displayMode === mode ? settingsAccentColor : '#fff',
                        },
                    }}
                >
    ...
    return (
        <Box className="settings-container" sx={{
            ...
            <Box sx={{
                ...
                borderRadius: isDefaultWhiteTheme ? '10px' : 0,
            }}>
                <Grid
                    container
                ...
                >
                    ...
                    {/* Sidebar - Mobile: Full width, Desktop: 2/12 */}

                    <Grid item xs={12} md={2} sx={{
                        order: { xs: 1, md: 1 },
                        p: isDefaultWhiteTheme ? 0 : { xs: 0.3, sm: 0.5, md: 1, lg: 1.5 },
                    ...
                    <Grid item xs={12} lg={10} sx={{
                        ...
                        {(displayMode === 'Client' || displayMode === 'Lutron' || displayMode === 'Project') && (
                            <Box
                                sx={{
                                    backgroundColor: isDefaultWhiteTheme ? 'transparent' : '#fff',
                                    borderRadius: isDefaultWhiteTheme ? 0 : { xs: '4px', sm: '6px', md: '8px', lg: '10px' },
                                    p: isDefaultWhiteTheme ? 0 : { xs: 0.5, sm: 0.8, md: 1.2, lg: 1.5 },
                                    width: '100%',
                                    maxWidth: isDefaultWhiteTheme ? '100%' : '92%',
                                    alignSelf: isDefaultWhiteTheme ? 'stretch' : 'center',
                                    display: 'flex',
                                    flexDirection: 'column',
                                    gap: { xs: 0.8, sm: 1, md: 1.2 },
                                }}
                            >
                                    ...
                                    >
                                        <Grid container spacing={{ xs: 0.5, sm: 0.8, md: 1.2, lg: 1.5 }} sx={{ flex: 1 }}>
                                                ...
                                                <Box sx={{
                                                    ...
                                                    <Button
                                                        ...
                                                        sx={{
                                                            ...
                                                            flexShrink: 0,
                                                            fontSize: { xs: '8px', sm: '10px', md: '12px', lg: '14px' },
                                                            backgroundColor: settingsAccentColor,
                                                            color: '#fff',
                                                            '&:hover': {
                                                                backgroundColor: isDefaultWhiteTheme ? SETTINGS_ACTION_BLUE_HOVER : darken(buttonColor, 0.12),
                                                            },
                                                        }}
                                                    >
                                                        Upload Image
                                            ...
                                            <Grid item xs={12} md={7}>
                                                ...
                                                <Box className="settings-home-quill" sx={settingsHomeQuillSx}>
                                                    <ReactQuill
                                                        value={description}
                                        ...
                                        >
                                            <Button
                                                ...
                                                sx={{
                                                    backgroundColor: saveLoading ? '#888' : settingsAccentColor,
                                                    color: '#fff',
                                                    '&:hover': { backgroundColor: saveLoading ? '#888' : (isDefaultWhiteTheme ? SETTINGS_ACTION_BLUE_HOVER : darken(buttonColor, 0.12)) },
                                                    borderRadius: '4px',
                                ...
                                {displayMode === 'Lutron' && (
                                    <Box className="lutron-content-scrollable" sx={settingsHomePanelSx}>
                                        ...
                                        <Typography fontWeight="bold" mt={2} mb={1} fontSize={{ xs: '9px', sm: '10px', md: '12px', lg: '14px' }}>
                                            Background Image
                                        </Typography>
                                        <Box sx={{
                                            display: 'flex',
                                        ...
                                        <Box
                                            ...
                                            justifyContent="flex-end"
                                            mt={1}
                                            sx={{ clear: 'both', position: 'relative', zIndex: 10 }}
                                        >
                                            <Button
                                                variant="contained"
                                                disabled={saveLoading}
                                ...
                                {displayMode === 'Project' && (
                                    <Box className="project-content-scrollable" sx={settingsHomePanelSx}>
                                        ...
                                        <Grid container spacing={{ xs: 0.5, sm: 0.8, md: 1.2, lg: 1.5 }} sx={{ mt: 0.5 }}>
