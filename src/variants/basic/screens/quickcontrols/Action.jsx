...
const Action = ({ areaId, onActionSelect, initialAction = null, hideZoneOption = false, hideSceneOption = false }) => {
  ...
  const sceneActiveBg = whiteChrome ? ACTION_BLUE : '#a89c81';
  const sceneInactiveBg = whiteChrome ? '#ffffff' : '#f7f4ed';
  const sceneItemBorder = whiteChrome ? '#d1d5db' : '#a89c81';
  const sceneActiveBorder = whiteChrome ? ACTION_BLUE : '#a89c81';
              ...
              renderItem={scene => {
                ...
                <li
                  ...
                  style={{
                    padding: "6px 10px",
                    marginBottom: 4,
                    borderRadius: whiteChrome ? 8 : 4,
                    background: isSelected ? sceneActiveBg : sceneInactiveBg,
                    color: isSelected ? "#fff" : whiteChrome ? "#000000" : "#222",
                    cursor: "pointer",
                    border: `1px solid ${isSelected ? sceneActiveBorder : sceneItemBorder}`,
                  }}
            ...
            {["disabled", "auto", "vacancy"].map((action) => (
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  fontSize: 16,
                  background: selectedOccupancy === action ? ACTION_BLUE : "#fff",
                  color: selectedOccupancy === action ? "#fff" : (whiteChrome ? "#000000" : buttonColor),
