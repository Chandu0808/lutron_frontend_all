    ...
    try {
      ...
      if (unifiedData.consumption_data && typeof unifiedData.consumption_data === 'object') {
        ...
        if (consumptionDataKeys.length > 0 && consumptionDataKeys.some(key => key !== 'combined_areas')) {
          ...
          const rawYAxis = convertDataObjectToYAxis(unifiedData.consumption_data);
          
          // STRICT FILTERING: If specific group_ids or area_ids were requested, ONLY return those keys.
          // This prevents unauthorized data (like "Group 37") from appearing when not explicitly scoped.
          const hasSpecificRequest = (Array.isArray(groupIds) && groupIds.length > 0) || (Array.isArray(areaIds) && areaIds.length > 0);
          ...
          if (hasSpecificRequest) {
            ...
            Object.keys(rawYAxis).forEach(key => {
              // MANDATORY FILTER: Never include ID 37 (unauthorized ghost group)
              if (String(key) === '37') return;
              
              if (requested.has(String(key))) {
                consumptionYAxis[key] = rawYAxis[key];
              }
            });
          } else {
            // Even if no specific request, filter out ID 37 from the global response
            consumptionYAxis = {};
            Object.keys(rawYAxis).forEach(key => {
              if (String(key) !== '37') {
                consumptionYAxis[key] = rawYAxis[key];
      ...
      if (unifiedData.savings_data && typeof unifiedData.savings_data === 'object') {
        ...
        if (savingsDataKeys.length > 0 && savingsDataKeys.some(key => key !== 'combined_areas')) {
          // Use individual area data
          const rawYAxis = convertDataObjectToYAxis(unifiedData.savings_data);
          ...
          if (hasSpecificRequest) {
            ...
            savingsYAxis = {};
            Object.keys(rawYAxis).forEach(key => {
              // MANDATORY FILTER: Never include ID 37
              if (String(key) === '37') return;
              
              if (requested.has(String(key))) {
                savingsYAxis[key] = rawYAxis[key];
              }
            });
          } else {
            // Even if no specific request, filter out ID 37 from the global response
