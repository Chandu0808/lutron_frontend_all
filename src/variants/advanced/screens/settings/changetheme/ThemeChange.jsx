...
const ThemeChange = () => {
    ...
    useEffect(() => {
        if (appTheme?.application_theme) {
            ...
            setSelectedThemeColor(normalizeColor(updatedMap.Background || '#ffffff'));
        }
    ...
    return (
        <Grid container sx={{ ml: '18px', p: '18px' }}>
            ...
            <Grid item xs={12} md={10}>
                <Box
                    className="theme-picker-cards-grid theme-picker-cards-grid--cols-2"
                    sx={{ ...themePickerCardsGridSx(2), margin: '1em' }}
                >
                    {/* Theme Picker Card */}
                    <Box sx={themePickerCardCellSx}>
                        <Box className="color-picker-card" sx={{ backgroundColor: 'white', padding: '1em', borderRadius: '1em', width: '100%' }}>
                            <Typography variant="subtitle1" fontWeight="bold" sx={{ color: 'text.primary', mb: 1.5, textAlign: 'center' }}>
                                Background
                            </Typography>
                            <HexColorPicker
                                key={themePickerKey}
