...
const Action = ({
  ...
  hideZoneOption = false,
  hideSceneOption = false,
}) => {
  ...
  const modalBodyColor = 'var(--schedule-modal-body-text, #222)';
  const modalMutedColor = 'var(--schedule-modal-muted-text, #3D4A5C)';
  const modalSelectedBg = 'var(--schedule-modal-item-selected-bg, #3D4A5C)';
  const modalSelectedText = 'var(--schedule-modal-item-selected-text, #ffffff)';
  const modalSectionLabelColor = 'var(--schedule-modal-section-label-color, inherit)';
  ...
  return (
    <div className="quick-control-modal" style={{ minWidth: 300, color: modalBodyColor, fontFamily: "inherit" }}>
      <Select
        id="action-select"
        ...
        fullWidth
        MenuProps={menuProps}
        sx={{ ...scheduleSelectFieldSx, mb: 2 }}
      >
        {!hideSceneOption && <MenuItem value="scene">Scene</MenuItem>}
        {!hideZoneOption && <MenuItem value="zone">Zone</MenuItem>}
      ...
      {selectedAction === "scene" && (
        <div>
          <div style={{ fontWeight: 600, marginBottom: 6, color: modalSectionLabelColor }}>Area Scenes</div>
          {Array.isArray(areaScenes) && areaScenes.length > 0 ? (
              ...
              renderItem={scene => (
                <li
                  ...
                  style={{
                    ...
                    marginBottom: 4,
                    borderRadius: 4,
                    // Previous tan/brown selected - kept for quick rollback:
                    // background selected #a89c81, unselected #f7f4ed, border #a89c81
                    background:
                      selectedScene &&
                      (selectedScene.id || selectedScene.scene_id) === (scene.id || scene.scene_id)
                        ? modalSelectedBg
                        : "#FFFFFF",
                    color:
                      selectedScene &&
                      (selectedScene.id || selectedScene.scene_id) === (scene.id || scene.scene_id)
                        ? modalSelectedText
                        : "var(--schedule-select-text, #1c2330)",
                    cursor: "pointer",
                    border:
                      selectedScene &&
                      (selectedScene.id || selectedScene.scene_id) === (scene.id || scene.scene_id)
                        ? `1px solid ${modalSelectedBg}`
                        : "1px solid #C5CDD8",
                    fontWeight:
                      selectedScene &&
                      (selectedScene.id || selectedScene.scene_id) === (scene.id || scene.scene_id)
            ...
            <div style={{ color: modalMutedColor, fontWeight: 500 }}>No scenes found for this area.</div>
          )}
        </div>
      )}
      ...
      {selectedAction === "zone" && (
        <div>
          <div style={{ fontWeight: 600, marginBottom: 6, color: modalSectionLabelColor }}>Zone</div>
          {zonesLoading && <div style={{ color: modalMutedColor }}>Loading zones...</div>}
            ...
            zones && zones.length > 0 ? (
              <>
                <Select
                  ...
                  displayEmpty
                  MenuProps={menuProps}
                  sx={{ ...scheduleSelectFieldSx, mb: 2, fontSize: 16 }}
                >
                  {getControllableZones().length >= 2 && (
                    <MenuItem value={ALL_ZONES_VALUE}>All Zones</MenuItem>
                  )}
                  {zones.map(z => (
                    <MenuItem key={z.id || z.zone_id} value={z.id || z.zone_id}>
                      {z.name}
                    </MenuItem>
                  ))}
                </Select>
              ...
              <div style={{ color: modalMutedColor, fontWeight: 500 }}>No zones found for this area.</div>
            )
          )}
        </div>
      )}
        ...
        <>
          ...
          <div style={{ marginBottom: 16 }}>
            <Select
              ...
              displayEmpty
              MenuProps={menuProps}
              sx={{ ...scheduleSelectFieldSx, fontSize: 16 }}
            >
              {shades.map(sh => (
                <MenuItem key={sh.id || sh.zone_id} value={sh.id || sh.zone_id}>
                  {sh.name}
                </MenuItem>
              ))}
            </Select>
