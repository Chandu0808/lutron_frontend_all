...
const FOFPMarkerInteractive = React.memo(
  function FOFPMarkerInteractive({
    ...
    const handleClick = (e) => {
      ...
      // Mouse click only — drop focus so the browser does not draw a black focus box on the marker.
    ...
    return (
      <g
        className="fofp-marker-interactive"
        data-testid={`fofp-marker-${p.zoneId}`}
        role="button"
        tabIndex={0}
        style={{ cursor: onZoneClick ? "pointer" : "default", outline: "none" }}
        pointerEvents="all"
        onClick={handleClick}
