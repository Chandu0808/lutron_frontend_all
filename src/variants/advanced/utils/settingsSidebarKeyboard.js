import { isKeyboardNavBlockedTarget } from "./keyboardNavUtils";
import { isSettingsAppRoute } from "./topbarNavActive";

/**
 * Module-level settings sidebar keyboard state — avoids stale React closures on route change.
 */
const api = {
  navItemKeys: [],
  navItems: [],
...
function handleSettingsSidebarWindowKeyDown(event) {
  if (!isSettingsAppRoute(api.pathname)) return;
